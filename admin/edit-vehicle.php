...
if($insuranceFile != null || $insuranceFile != ""){
  $sqlif="update tblvehicle set insurance=:insurance where ID=:vehid";
  $queryif = $dbh -> prepare($sqlif);
  $queryif->bindParam(':insurance',$insuranceName,PDO::PARAM_STR);
  $queryif->bindParam(':vehid',$vehid,PDO::PARAM_STR);
  $lastInsertId=$queryif -> execute();
  move_uploaded_file($insuranceFile["tmp_name"], $insurancePath);
}

if($vehicalLicenseFile != null || $vehicalLicenseFile != ""){
  $sqlvl="update tblvehicle set vehical_lecense=:vehical_lecense where ID=:vehid";
  $queryvl = $dbh -> prepare($sqlvl);
  $queryvl->bindParam(':vehical_lecense',$licenseName,PDO::PARAM_STR);
  $queryvl->bindParam(':vehid',$vehid,PDO::PARAM_STR);
  $lastInsertId=$queryvl -> execute();
...
{               ?>
                ...
                <div class="form-group col-md-12">
                  <label class="control-label"> Vehicle Licenses
                    <a href="../images/<?php echo $result->vehical_lecense;?>"> Click here to view current Vehical Licenses</a>
                  </label>
                ...
                <label class="control-label"> Insurance
                    <a href="../images/<?php echo $result->insurance;?>"> Click here to view current insurance</a>
                  </label>
